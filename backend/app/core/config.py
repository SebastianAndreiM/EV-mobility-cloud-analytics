...
A single DATABASE_URL is provided by the env FastAPI needs the
asyncpg driver, Celery worker needs a sync psycopg2 driver, so we
...
class Settings(BaseSettings):
    ...
    #Redis /0 rate-limit, /1 celery results, /2 cache  (separate indices, no collisions)
